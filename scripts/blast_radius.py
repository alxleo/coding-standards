...
def print_coupling(couplings: list[dict]) -> None:
    ...
    for c in couplings[:20]:
        print(
            f"{c['file_a'][:45]:<45}  {c['file_b'][:45]:<45}"
            f"  {c['co_changes']:>6}  {c['coupling']:>7.1%}"
        )
...
def print_criticality(ranked: list[dict], top: int = 20) -> None:
    ...
    for r in ranked[:top]:
        print(
            f"{r['file'][:55]:<55}  {r['criticality']:>8.4f}"
            f"  {r['in_degree']:>2}  {r['out_degree']:>3}"
        )
...
def print_pr_review(review: dict) -> None:
    ...
    if review["possibly_missing"]:
        ...
        for m in review["possibly_missing"]:
            print(
                f"  {m['missing']} ↔ {m['coupled_to']}"
                f" ({m['coupling']:.0%} coupling, {m['co_changes']} co-changes)"
            )
    else:
