...
def show_warnings(report_path: Path) -> int:
    ...
    for linter in report.get("linters", []):
        errors = linter.get("total_number_errors", 0) or 0  # nosemgrep: coding-standards.python-silent-fallback-or
        # Skip linters with no findings or that hard-failed (error-tier)
        if errors == 0 or linter.get("status") == "error":  # nosemgrep: coding-standards.python-silent-fallback-or
            continue
