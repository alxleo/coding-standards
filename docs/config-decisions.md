...
### Custom check categories (reusable patterns for consumer repos)
Six categories of custom checks, mapped to three reusable mechanisms:

Seven categories of custom checks. Project-specific parts are DATA (fields, paths, patterns), never MECHANISM.

| # | Category | Mechanism | Example | In image? |
|---|----------|-----------|---------|-----------|
| 1 | Structural validation | Conftest (Rego policies) | compose must have resource limits, labels match schema | Yes (binary) |
| 2 | Convention enforcement | Semgrep custom rules | no bare python3, pin npm versions, temp files need trap | Yes (already present) |
| 3 | Generated file drift | Generic wrapper script | `generate-X --check && git diff --exit-code` | Yes (wrapper) |
| 4 | Cross-file referential integrity | Generic framework | refs in file A must exist in file B | Yes (framework) |
| 5 | Expiry enforcement | Generic date-check | `REMOVE_AFTER: 2026-04-01` past due | Yes (script) |
| 6 | Config invariant validator | Generic harness + check DSL | router config required sections, grafana alert rules | Yes (harness) |
| 7 | Taint detection (non-standard langs) | Regex-based taint checker | unvalidated vars in ssh/sudo commands in justfiles | Yes (script) |

Consumer repos provide: Rego policies in `policy/`, semgrep rules in `.semgrep/`, check definitions in `.coding-standards.yml` or config files. Zero custom scripts needed.

Validated against home-network's 37 custom hooks: every one fits a category.
